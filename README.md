...
6. **코딩 및 디자인 가이드 작성 :** 개발 및 디자인 표준 설정
    ...
    - 클래스 네임은 카멜 표기법으로 쉽게 알아볼 수 있는 이름으로 작성하고 네 단어 이상의 사용을 지양한다.
    - const 선언방식을 사용한다.
    - 따옴표는 큰 따옴표를 사용한다.
    - 이벤트 핸들러 함수를 사용한다.
    - Prettier를 사용한다.
...
### 상황 1
  ...
  - #### 문제 확인:
     브라우저 내에서 데이터를 저장하는 로컬스토리지를 통해 데이터를 저장하거나 불러올 수 있었어요
...
### 상황 3
  ...
  - #### 문제 확인:
     원인 파악이 되어있지않아 직접 확인 해봤어요 수정 컴포넌트를 모달창으로 띄우는 방식에서 데이터를 불러오는 경로가 꼬인거 같다고 추측했어요
      
  - #### 해결:
    1. 결국 코드를 싹 다 갈아엎고 처음부터 다시 작성했어요
    2. useEffect Hook을 사용하여 로컬 스토리지와 상태 업데이트 관리했어요
    3. 로컬 스토리지에 'foodList'라는 키로 식품 목록을 문자열 형태로 저장하고 foodList에서 주어진 food와 같은 항목을 찾아 그 인덱스를 반환하며 해결 했어요
...
### 상황 4
  ...
  - #### 문제 확인:
     몽고DB를 이용하여 서버 연결을 통해 회원정보와 데이터 정보를 연결하기로 했어요
...
### 상황 5
  ...
  - #### 문제 확인:
     서로 낯을 많이 가리는 팀원들이었어요
      
  - #### 해결:
     용기를 내 팀원들에게 다가가 단톡방을 만들고 먼저 대화를 주도하여 친밀감을 형성시키고 밝은 분위기로 회의를 이끌어 좋은 분위기에서 프로젝트를 마칠 수 있었어요


## 프로젝트 완료 리뷰
- ### 아쉬웠던 점:
  #### 리액트를 활용함에 어렵고 서툴렀던 탓에 프로젝트 진행 속도가 늦어졌어요
  #### 낯을 많이 가리는 성격이라 팀원들과 좋은 팀워크를 이루지 못할 거라 생각했어요
